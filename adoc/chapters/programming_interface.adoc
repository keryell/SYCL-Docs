...
----
   a@ An explicit specialization of [code]#is_property# that inherits
      ...
      where [code]#propertyT# is the class defining the property.
...
----
   a@ Variable containing value of [code]#is_property<propertyT>#.
...
----
   a@ An explicit specialization of [code]#is_property_of# that
      ...
      [code]#propertyT# is the class defining the property and
      [code]#syclObjectT# is the SYCL class. This includes both
...
----
   a@ Variable containing value of [code]#is_property_of<propertyT, syclObjectT>#.
...
----
   a@ Returns true if [code]#T# was constructed with the property
      specified by [code]#propertyT#. Returns false if it was
      not.
...
----
   a@ Returns a copy of the property of type [code]#propertyT#
...
[source]
----
template <typename T_src, int dim_src, access_mode mode_src,
         target tgt_src, access::placeholder isPlaceholder,
         typename T_dest>
...
----
template <typename T_src, int dim_src, access_mode mode_src,
          target tgt_src, access::placeholder isPlaceholder_src,
          typename T_dest, int dim_dest, access_mode mode_dest,
          target tgt_dest, access::placeholder isPlaceholder_dest>
event copy(
...
[code]#id<int _dimensions_># is a vector of dimensions that is used to
...
----
   a@ This function must only be used to invoke a kernel that was constructed
      ...
      error code.  The precise semantics of this function are defined by each
      SYCL backend specification, but the intent is that the kernel should be
      invoked for the specified [code]#ndRange#.
...
----
template <typename T_src, int dim_src, access_mode mode_src,
          target tgt_src, typename T_dest,
          access::placeholder isPlaceholder>
...
template <typename T, int dim,
          ...
          access::placeholder isPlaceholder>
void update_host(accessor<T, dim,
                          mode, tgt,
                          isPlaceholder> acc)
----
